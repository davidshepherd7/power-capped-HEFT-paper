...
Our algorithm is based on the well known HEFT scheduling algorithm (Heterogeneous Earliest Finish Time) \cite{Topcuoglu2002heft}.
In standard HEFT initially all VMs are available at all times, and slots are occupied by jobs as the algorithm progresses.
In our power-capped modification we instead begin with some slots occupied by dummy jobs at times when there is insufficient power to run that VM.
Following this initial modification of the available slots the algorithm proceeds as standard HEFT.
Full details are given in \cref{alg:power-capped-heft}.
...
\begin{algorithm}
  \begin{algorithmic}[1]
    \State Determine number of VMs as a function of time
    \State Allocate dummy jobs to times when VMs are not available
    \State Run HEFT
  \end{algorithmic}
  \caption{Power capped HEFT}
