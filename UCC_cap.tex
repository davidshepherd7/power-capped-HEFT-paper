...
\Cref{fig:sipht} shows the ratio of the two makespans against the number of nodes in the DAG for SIPHT workflows. \Cref{fig:genome} shows the same plot for GENOME workflows.
We see that in both cases the HEFT-based scheduling algorithm outperforms the naive approach by a margin between roughly $0.9\times$ and $0.7\times$.
...
\newcommand{\captionmacro}[1]{The ratio of the HEFT-based makespan to the FCFS-based makespan against the number of nodes in the workflow DAG for {{#1}} workflows \cite{Bharathi2008}.}

\begin{figure}
  \centering
  \includegraphics[width=\columnwidth]{{{images/schedule-length-ratio-SIPHT}}}
  \caption{\captionmacro{SIPHT}}
  \label{fig:sipht}
\end{figure}
...
\begin{figure}
  ...
  \caption{\captionmacro{GENOME}}
  \label{fig:genome}
\end{figure}

\section{Conclusion}

We have introduced an algorithm for resource provisioning and workflow scheduling under time-varying power constraints based.
Our algorithm offers around a $0.8\times$ improvement in the makespan over the naive approach of using dynamic scheduling without knowledge of the changing power cap.
