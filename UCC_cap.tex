...
\Cref{fig:placeholder,fig:placeholder1} show normalised makespans against number of nodes in the DAG for two example applications when scheduled with power-capped HEFT and the simple FCFS approach.
The makespan (total run time), $T$, is normalised by $T' = \alpha \int_0^T P(t) \d t$, where $\alpha$ is as above.
$T'$ corresponds to the optimal makespan if all available energy from the time-varying power cap was used.
